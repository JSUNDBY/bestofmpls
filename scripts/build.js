...
// The Living Best of: generated by scripts/livingbest.js (gitignored). Missing
// is fine — the page just reads as "still gathering signal".
// Curated opening receptions (verified; fed by /submit-opening/ via the admin inbox)
const artOpenings = (function(){ try { return require(path.join(SRC, 'data/art-openings.json')); } catch (_) { return { openings: [] }; } })();

const livingBest = (function(){ try { return require(path.join(SRC, 'data/livingbest.json')); } catch (_) { return { places: {}, standings: {}, total_voices: 0, generated_at: '' }; } })();
// Optional editorial notes per award category slug (Josh fills these in).
const bestOfNotes = (function(){ try { return require(path.join(SRC, 'data/bestof-notes.json')); } catch (_) { return {}; } })();
...
// Drop exhibitions whose end date has passed, at build time, so /now-showing/
// can never recommend a closed show no matter how stale the data file gets.
// Entries with unparseable/recurring dates ("First Thursday...") are kept.
function exhibitionEndDate(dates) {
  if (!dates) return null;
  const M = 'January|February|March|April|May|June|July|August|September|October|November|December';
  let m = dates.match(new RegExp(`(?:to|through)\\s+(${M})\\s+(\\d{1,2}),\\s*(\\d{4})`, 'i'));
  if (m) return new Date(`${m[1]} ${m[2]}, ${m[3]} 23:59:59`);
  // "May 15 to 17, 2026" — the end day shares the start month
  m = dates.match(new RegExp(`(${M})\\s+\\d{1,2}\\s+to\\s+(\\d{1,2}),\\s*(\\d{4})`, 'i'));
  if (m) return new Date(`${m[1]} ${m[2]}, ${m[3]} 23:59:59`);
  m = dates.match(/through\s+(\d{4})/i);
  if (m) return new Date(`December 31, ${m[1]} 23:59:59`);
  return null;
}
{
  const now = new Date();
  const before = exhibitions.exhibitions.length;
  exhibitions.exhibitions = exhibitions.exhibitions.filter((e) => {
    const end = exhibitionEndDate(e.dates);
    if (end && end < now) { console.log(`  now-showing: dropped closed show "${e.title}" (${e.dates})`); return false; }
    return true;
  });
  if (before !== exhibitions.exhibitions.length) {
    console.log(`  now-showing: ${before - exhibitions.exhibitions.length} closed shows filtered, ${exhibitions.exhibitions.length} remain`);
  }
}
...
const NEIGHBORHOODS = [
  ...
  { slug: 'uptown-lyn-lake', name: 'Uptown & Lyn-Lake', short: 'Uptown / Lyn-Lake', match: /(lyn-lake|uptown|kingfield)/i, intro: 'Mortimer\'s, Bryant Lake Bowl, Khâluna, and a stretch of Lyndale Avenue that still anchors a lot of Minneapolis nightlife. Less polished than it was, more interesting in some ways.' },
...
// The Living Best of — participation. Anonymous device id in localStorage; taps
// post to the worker /signal endpoint (fire-and-forget, optimistic UI).
(function(){
  var WORKER = ${JSON.stringify(POLL_WORKER_URL || '')};
  if (!WORKER) return;
  function dev(){ try { var d = localStorage.getItem('bom_dev'); if (!d) { d = Date.now().toString(36) + Math.random().toString(36).slice(2, 10); localStorage.setItem('bom_dev', d); } return d; } catch (e) { return 'anon'; } }
  function getMap(k){ try { return JSON.parse(localStorage.getItem(k) || '{}'); } catch (e) { return {}; } }
  function setMap(k, o){ try { localStorage.setItem(k, JSON.stringify(o)); } catch (e) {} }
  function send(place, type, extra){ try { fetch(WORKER + '/signal', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(Object.assign({ type: type, place: place, device: dev() }, extra || {})), keepalive: true }).catch(function(){}); } catch (e) {} }

  // Save / I'm a regular / Share a moment
  ...
  // Directions taps = real visit intent.
  document.querySelectorAll('[data-lb-directions]').forEach(function(a){
    a.addEventListener('click', function(){
      var place = a.getAttribute('data-lb-place'); if (!place) return;
      var body = JSON.stringify({ type: 'directions', place: place, device: dev() });
      try { if (navigator.sendBeacon) navigator.sendBeacon(WORKER + '/signal', new Blob([body], { type: 'application/json' })); else send(place, 'directions'); } catch (e) {}
    });
  });
})();
</script>
...
function renderEntry(c, e, allCategories) {
  ...
  const lbActionsBlock = POLL_WORKER_URL ? `
    <div class="lb-actions" data-lb-place="${c.slug}/${slug}">
      ...
      <form class="lb-story" data-lb-story hidden>
        <textarea class="lb-story-text" maxlength="1000" rows="3" placeholder="A moment you had here — the meal, the night, why it matters."></textarea>
...
function renderExhibitions() {
  ...
  const openingsSection = openingsList.length ? `
    <div class="exhibition-section" id="openings">
      <div class="wrap">
        <h2 class="exhibition-section-title">Openings</h2>
        <p class="openings-deck">The nights the art world is actually in the room. Go to one; nobody checks credentials at a gallery door.</p>
      </div>
      <div class="wrap openings-list">
        ${openingsList.map(o => `
        <article class="opening-row">
          <div class="opening-when">
            <span class="opening-date">${esc(fmtOpen(o.date))}</span>
            ${o.time ? `<span class="opening-time">${esc(fmt12h(o.time))}${o.end_time ? '\u2013' + esc(fmt12h(o.end_time)) : ''} \u00b7 reception</span>` : (o.reception ? `<span class="opening-time">opening reception</span>` : `<span class="opening-time opening-time--day">first day on view</span>`)}
          </div>
          <div class="opening-body">
            <h3 class="opening-title">${o.url ? `<a href="${esc(o.url)}" target="_blank" rel="noopener">${esc(o.title)}</a>` : esc(o.title)}</h3>
            <div class="opening-venue">${esc(o.venue)}${o.artist ? ' \u00b7 ' + esc(o.artist) : ''}${o.on_view_through ? ` \u00b7 on view through ${esc(fmtOpen(o.on_view_through).replace(/^[A-Za-z]+, /, ''))}` : ''}</div>
          </div>
        </article>`).join('')}
      </div>
    </div>` : '';
  const submitCta = `
    <div class="wrap">
      <aside class="openings-cta">
        <h2 class="openings-cta-title">Run a gallery or project space?</h2>
        <p class="openings-cta-body">Send us your openings and receptions and we will put them on this page and in front of the metro. Free, takes two minutes, and we verify everything before it runs.</p>
        <a class="cover-cta" href="/submit-opening/">Submit an opening \u2192</a>
      </aside>
    </div>`;

  const sections = [];
  if (openingsSection) sections.push(openingsSection);
  sections.push(submitCta);
  if (current.length) {
    sections.push(`
...
function renderQuiz() {
  ...
  const neighborhoods = [
    { slug: 'northeast-minneapolis', label: 'Northeast Minneapolis', short: 'Northeast', deck: 'Old breweries, working artist studios, the densest run of independent restaurants in the metro.', picks: ['Dinner at Vinai', 'Indeed Brewing patio', 'Northrup King artist studios'] },
    ...
    { slug: 'uptown-lyn-lake', label: 'Uptown / Lyn-Lake', short: 'Lyn-Lake', deck: 'Where Minneapolis nightlife still lives, less polished than it was, more interesting in some ways.', picks: ['Khâluna', "Mortimer's", 'Bryant Lake Bowl for dinner and a show'] },
...
// The Living Best of is live: the page builds off real reader signal + the
// honest accolade floor, ribbons show on current leaders.
const BEST_OF_LIVE = true;
const AWARDS = [
  { group: 'Eat', items: [
...
// The ribbon only shows on the current live leader, and only once the category
// has real reader signal — never on a place that just has the accolade floor.
function isBestOfWinner(slug, name) {
  const r = livingRankOf(slug, name);
  return !!r && r.rank === 1 && r.voices > 0;
}
...
// Live ranking per award category, off the Living Best of score. Map of
// placeId -> { rank, total, voices, award, score }. Recomputed each build.
const LIVING_RANK = (function(){
  const map = {};
  for (const g of AWARDS) for (const [slug, award] of g.items) {
    const c = categories.find(x => x.slug === slug);
    if (!c || !c.entries || !c.entries.length) continue;
    const ranked = c.entries.map(e => {
      const pid = `${slug}/${entrySlug(e.name)}`;
      const lb = livingBest.places[pid] || {};
      return { pid, score: lb.score || 0, voices: lb.voices || 0 };
    }).sort((a, b) => b.score - a.score);
    ranked.forEach((r, i) => { map[r.pid] = { rank: i + 1, total: ranked.length, voices: r.voices, award, score: r.score }; });
  }
  return map;
})();
...
function renderBestOf() {
  ...
  const breakdown = lb => {
    const p = [];
    if (lb.nRegulars) p.push(`${lb.nRegulars} regular${lb.nRegulars === 1 ? '' : 's'}`);
    if (lb.nSaves) p.push(`${lb.nSaves} save${lb.nSaves === 1 ? '' : 's'}`);
    if (lb.nStories) p.push(`${lb.nStories} stor${lb.nStories === 1 ? 'y' : 'ies'}`);
    return p.join(' · ');
  };
  const groups = AWARDS.map(g => ({
    ...
    items: g.items.map(([slug, award]) => {
      ...
      const ranked = c.entries.map(e => {
        const lb = livingBest.places[`${slug}/${entrySlug(e.name)}`] || {};
        return { e, lb, score: lb.score || 0, voices: lb.voices || 0 };
      }).sort((a, b) => b.score - a.score);
      const catVoices = ranked.reduce((n, r) => n + r.voices, 0);
      return { award, slug, c, winner: ranked[0], catVoices };
    }).filter(Boolean)
  ...
  const body = groups.map(g => `
    <section class="wrap bestof-group">
      ...
      <div class="bestof-list">
        ${g.items.map(it => {
          const live = it.catVoices > 0;
          const note = bestOfNotes[it.slug];
          const w = it.winner;
          return `
          <div class="bestof-cell">
            <a class="bestof-award" href="/${it.c.slug}/${entrySlug(w.e.name)}/">
              <span class="bestof-award-label">${esc(it.award)}${live ? '' : ' · early'}</span>
              <span class="bestof-award-winner">${esc(w.e.name)}</span>
              ${w.e.neighborhood ? `<span class="bestof-award-hood">${esc(w.e.neighborhood)}</span>` : ''}
              ${w.e.description ? `<span class="bestof-award-why">${esc(firstSentence(w.e.description))}</span>` : ''}
              <span class="bestof-award-signal${live ? '' : ' bestof-award-signal--early'}">${live ? esc(breakdown(w.lb) || `${w.voices} voices`) : 'accolade pick · your taps decide'}</span>
            </a>
            ${note ? `<p class="bestof-note"><strong>Editor:</strong> ${esc(note)}</p>` : ''}
          </div>`;
        }).join('')}
      </div>
    </section>`).join('');

  const stamp = esc(livingBest.generated_at || UPDATED_LABEL);
  const voicesLine = totalVoices > 0
    ? `Updated ${stamp} · shaped by ${totalVoices} local${totalVoices === 1 ? '' : 's'} so far`
    : `Updated ${stamp} · just getting started, your taps shape it`;

  const itemList = {
    '@context': 'https://schema.org', '@type': 'ItemList', name: 'Best of MPLS',
    itemListElement: groups.flatMap(g => g.items).map((it, i) => ({
      '@type': 'ListItem', position: i + 1, name: `${it.award}: ${it.winner.e.name}`,
      url: `${SITE}/${it.c.slug}/${entrySlug(it.winner.e.name)}/`
    }))
  };

  return head({ title: 'Best of MPLS: the metro\'s living best, shaped by the people who go', description: `The Twin Cities' best across ${total} categories, decided by what locals actually save, return to, and tell stories about. Updated continuously, independent, never pay-to-play.`, slug: 'best-of-2026', theme: 'clay' }) +
    header({ activeSlug: '' }) +
    `<section class="section-head">
       <div class="wrap">
         <div class="section-eyebrow">Living · ${total} categories</div>
         <h1 class="section-title">Best of MPLS</h1>
         <p class="section-deck">Not a frozen list, and not one critic's verdict. The metro's best, shaped by what locals actually save, return to, and tell stories about, weighted to right now.</p>
         <p class="bestof-voices">${voicesLine}</p>
       </div>
     </section>
     ${body}
...
// Gallery opening submissions — the free community pipeline for /now-showing/
// (#openings). Posts to the worker /tip tagged OPENING SUBMISSION; Josh
// verifies against the gallery's own announcement, then adds the entry to
// src/data/art-openings.json.
function renderSubmitOpening() {
  const description = 'Galleries and project spaces: send Best of MPLS your opening receptions and exhibitions. Free, verified, and listed on the openings calendar.';
  const enabled = !!POLL_WORKER_URL;
  return head({ title: 'Submit an opening', description, slug: 'submit-opening', theme: 'default' }) +
    header({ activeSlug: '' }) +
    `<section class="section-head">
      <div class="wrap">
        <div class="section-eyebrow">For galleries and project spaces</div>
        <h1 class="section-title">Submit an opening</h1>
        <p class="section-deck">Tell us about your reception or exhibition and we will list it on <a href="/now-showing/#openings">the openings calendar</a>. Free, always. We verify every submission against your own announcement before it runs, so include a link.</p>
      </div>
    </section>
    <section class="wrap">
      <form class="contribute-form" data-open-form>
        <div>
          <label for="o-venue">Gallery or space</label>
          <input id="o-venue" name="venue" type="text" required maxlength="120" placeholder="e.g. Dreamsong, TOA Presents, your studio building">
        </div>
        <div>
          <label for="o-email">Your email</label>
          <input id="o-email" name="email" type="email" required maxlength="200" placeholder="So we can confirm details">
        </div>
        <div>
          <label for="o-title">Show title and artist(s)</label>
          <input id="o-title" name="title" type="text" required maxlength="200" placeholder="e.g. New Paintings, Kim Benson">
        </div>
        <div>
          <label for="o-when">Opening reception (date and time)</label>
          <input id="o-when" name="when" type="text" required maxlength="120" placeholder="e.g. Friday, September 26, 6-9pm">
        </div>
        <div>
          <label for="o-run">On view through</label>
          <input id="o-run" name="run" type="text" maxlength="120" placeholder="e.g. November 8">
        </div>
        <div>
          <label for="o-link">Link to your announcement</label>
          <input id="o-link" name="link" type="url" maxlength="300" placeholder="Your site or Instagram post \u2014 this is how we verify">
        </div>
        <input type="text" name="hp" tabindex="-1" autocomplete="off" class="poll-hp" aria-hidden="true">
        <div class="poll-actions">
          <button class="cover-cta" type="submit">Send the opening \u2192</button>
          <span class="poll-status" data-open-status></span>
        </div>
      </form>
      <div class="poll-thanks" data-open-thanks hidden>
        <h3 class="poll-thanks-title">Got it.</h3>
        <p class="poll-thanks-body">We verify against your announcement and list it, usually within a day. Send every show \u2014 once we know you, it only gets faster.</p>
      </div>
      <div class="about-body" style="padding-top: 24px;">
        <p style="font-style: italic; color: var(--ink-soft);">Want more than a listing? Featured placement and video profiles live on <a href="/partner/">the partner page</a>.</p>
      </div>
    </section>
    ${enabled ? `<script>
      (function(){
        var form = document.querySelector('[data-open-form]');
        if (!form) return;
        var status = document.querySelector('[data-open-status]');
        var thanks = document.querySelector('[data-open-thanks]');
        form.addEventListener('submit', async function(e){
          e.preventDefault();
          var fd = new FormData(form);
          status.textContent = 'Sending...';
          ...
          try {
            var res = await fetch(${JSON.stringify(POLL_WORKER_URL + '/tip')}, {
              method: 'POST', headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                name: 'OPENING SUBMISSION',
                email: fd.get('email'),
                place: fd.get('venue'),
                message: 'Show: ' + fd.get('title') + '\\nReception: ' + fd.get('when') + '\\nOn view through: ' + (fd.get('run') || '?') + '\\nLink: ' + (fd.get('link') || 'none given'),
                hp: fd.get('hp')
              })
            });
            var data = await res.json();
...
function build() {
  ...
  writeFile('submit-opening/index.html', renderSubmitOpening());
